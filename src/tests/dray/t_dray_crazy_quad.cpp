...
TEST (dray_crazy_quad, dray_crazy_quad_convert)
{
  ...
  std::string (DATA_DIR) + "CrazyQuadData/CrazyQuadData";
  ...
  conduit::utils::join_file_path (prepare_output_dir (), output_visit_dc);
  ...
  // If output right here with visit_dc.Save(), will get exactly the input, not
  // positive. Verified that mfem_mesh_ptr == visit_dc.GetMesh().
