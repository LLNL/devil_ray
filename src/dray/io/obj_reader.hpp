...
inline void ObjReader::loadMTL(const std::string &mtlFilename)
{
    ...
    while (ifs.peek() != EOF) {
        ...
        if (!mat) 
        {
            std::cout<<"Bad line in mat file : "<<line<<std::endl;
...
inline void ObjReader::extractNormals()
{
    ...
    for(int i=0; i<size;i++)
    {
        ...
        {   
            ...
            Vec3fobj *norm;
            if(model[i]->mesh==NULL) std::cerr<<" mesh"<<model[i]->mesh<<std::endl;
            for(int j=0;j<s;j++)
            {
                //extract the normals
                int v0,v1,v2;
            ...
            for (int j=0;j<count;j++)
            {
                ...
                if(m==0) {std::cerr<<"DIVIDE BY ZER0"<<std::endl; continue;}
...
inline void ObjReader::getRawData(dray::Array<dray::float32> &a_verts, dray::Array<dray::int32> &a_indices)
{
    ...
    {
      std::cerr<<"No Triangles to output"<<std::endl;
      //verts=NULL;
    }
}
