...
template <int32 PhysDim> struct ElTransIter
{
  static constexpr int32 phys_dim =
  PhysDim; // TODO might have to define this in the implementation file as well.
  ...
  DRAY_EXEC void
  init_iter (const int32 *ctrl_idx_ptr, const Vec<Float, PhysDim> *val_ptr, int32 el_dofs, int32 el_id)
  {
  ...
  DRAY_EXEC void operator+= (int32 dof_offset)
  {
    m_offset += dof_offset;
  } // Less expensive
...
DRAY_EXEC ElTransIter<PhysDim> ElTransIter<PhysDim>::operator+ (int32 dof_offset) const
{
...
//
template <int32 PhysDim> struct ElTransBdryIter : public ElTransIter<PhysDim>
{
