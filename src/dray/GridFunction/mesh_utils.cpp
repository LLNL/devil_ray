...
BVH construct_bvh (Mesh<ElemT> &mesh,
                   Array<typename get_subref<ElemT>::type> &ref_aabbs)
{
...
{
  // - the old geometry array will be re-used, so dof ids
...
template
GridFunction<1> extract_face_dofs(const ShapeHex,
                                  const GridFunction<1> &orig_data_3d,
                                  const int32 poly_order,
                                  const Array<Vec<int32, 2>> &elid_faceid);
template
GridFunction<3> extract_face_dofs(const ShapeHex,
                                  const GridFunction<3> &orig_data_3d,
                                  const int32 poly_order,
                                  const Array<Vec<int32, 2>> &elid_faceid);

template
GridFunction<1> extract_face_dofs(const ShapeTet,
                                  const GridFunction<1> &orig_data_3d,
                                  const int32 poly_order,
                                  const Array<Vec<int32, 2>> &elid_faceid);
template
GridFunction<3> extract_face_dofs(const ShapeTet,
                                  const GridFunction<3> &orig_data_3d,
                                  const int32 poly_order,
                                  const Array<Vec<int32, 2>> &elid_faceid);
