...
///
template <typename T>
DRAY_EXEC dray::Vec<T, 3> transform_point(const dray::Matrix<T, 4, 4>& matrix,
                                          const dray::Vec<T, 3>& point)
{
...
///
template <typename T>
DRAY_EXEC dray::Vec<T, 3> transform_vector(const dray::Matrix<T, 4, 4>& matrix,
                                           const dray::Vec<T, 3>& vector)
{
  dray::Vec<T, 4> homogeneousVector{vector[0], vector[1], vector[2], T(0)};
  homogeneousVector = matrix * homogeneousVector;
  return dray::Vec<T, 3>{homogeneousVector[0], homogeneousVector[1], homogeneousVector[2]};
}
...
///
template <typename T>
DRAY_EXEC dray::Matrix<T, 4, 4> scale(const T& scaleX,
                                      const T& scaleY,
                                      const T& scaleZ)
{
...
///
template <typename T>
DRAY_EXEC dray::Matrix<T, 4, 4> rotate(T angleDegrees,
                                       const dray::Vec<T, 3>& axisOfRotation)
{
...
///
template <typename T>
DRAY_EXEC dray::Matrix<T, 4, 4> rotate_x(T angleDegrees)
{
...
///
template <typename T>
DRAY_EXEC dray::Matrix<T, 4, 4> rotate_y(T angleDegrees)
{
...
///
template <typename T>
DRAY_EXEC dray::Matrix<T, 4, 4> rotate_z(T angleDegrees)
{
  return dray::rotate(angleDegrees, T(0), T(0), T(1));
}

static DRAY_EXEC
dray::Matrix<float32, 4, 4> trackball_matrix(dray::float32 p1x,
                                                   dray::float32 p1y,
                                                   dray::float32 p2x,
                                                   dray::float32 p2y)
{
  ...
  t = 1.0f /
    static_cast<float32>(sqrt(q[0] * q[0] + q[1] * q[1] + q[2] * q[2] + q[3] * q[3]));
  q[0] *= t;
  q[1] *= t;
  q[2] *= t;
  q[3] *= t;
