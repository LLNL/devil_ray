...
AABB<3> bound_sphere(const Vec<Float,3> &center, const Float radius)
{
  AABB<3> res;
  Vec<Float,3> temp;
...
{
  const Float *m_texture;
  const int32 m_texture_width;
  const int32 m_texture_height;
  const Vec<Float,3> m_ray_diff_x;
  const Vec<Float,3> m_ray_diff_y;
  const Vec<Float,3> *m_anchors;
  const Vec<Float,2> *m_offsets;
  const Vec<Float,2> *m_dims;
  const Vec<Float,2> *m_tcoords;
  const Vec<Float,3> m_up;
  const Vec<Float,3> m_text_color;
  ...
  {
    const Vec<Float,3> anchor = m_anchors[index];
    const Vec<Float,2> dims = m_dims[index];
    //Ray tracing Gems II billboard intersections
    Vec<Float,3> normal_dir = ray.m_orig - anchor;
    bool y_align = false;
    Vec<Float,3> n = normal_dir;
    if(y_align)
    {
    ...
    n.normalize();
    Vec<Float,3> t = cross(m_up, n);
    t.normalize();
    Vec<Float,3> b = cross(n, t);

    Matrix<Float,3,3> to_tangent;
    to_tangent.set_col(0,t);
    to_tangent.set_col(1,b);
    to_tangent.set_col(2,n);
    to_tangent = to_tangent.transpose();

    Vec<Float,3> dp = to_tangent * ray.m_dir;
    Vec<Float,3> op = to_tangent * normal_dir;
    Float s = -op[2] / dp[2];
    Vec<Float,2> pp; // point on billboard plane
    pp[0] = op[0] + s * dp[0];
    pp[1] = op[1] + s * dp[1];
    ...
    // the billboard
    const Vec<Float,2> offset = m_offsets[index];
  ...
  DRAY_EXEC
  Vec<Float,2> lerp_tbox(const AABB<2> &tbox, const Vec<Float,2> uv) const
  {
    Float s = lerp(tbox.m_ranges[0].min(),tbox.m_ranges[0].max(), uv[0]);
    Float t = lerp(tbox.m_ranges[1].min(),tbox.m_ranges[1].max(), uv[1]);
    Vec<Float,2> st = {{s,t}};
    return st;
  }
  ...
  void texture_derivs(const Ray &ray,
                      const RayHit &hit,
                      const AABB<2> &tbox,
                      const Vec<Float,2> &st,
                      Vec<Float,2> &st_x,
                      Vec<Float,2> &st_y) const
  {
    Ray ray_x = ray;
    Ray ray_y = ray;
    ...
    {
      Vec<Float,2> uv_x = {{hit_x.m_ref_pt[0], hit_x.m_ref_pt[1]}};
    ...
    {
      Vec<Float,2> uv_y = {{hit_y.m_ref_pt[0], hit_y.m_ref_pt[1]}};
  ...
  DRAY_EXEC
  Float tblerp(const Float s, const Float t) const
  {
    ...
    Float dx = s - Float(st_min[0]);
    Float dy = t - Float(st_min[1]);

    Float x0 = lerp(vals[0], vals[1], dx);
    Float x1 = lerp(vals[2], vals[3], dx);
    // this the signed distance to the glyph
    return lerp(x0, x1, dy);
  }

  Float alpha(const Ray &ray, const RayHit &hit) const
  {
    if(hit.m_hit_idx == -1)
    {
      return 0.f;
    ...
    Vec<Float,2> uv = {{hit.m_ref_pt[0], hit.m_ref_pt[1]}};

    Vec<Float,2> st = lerp_tbox(tbox, uv);
    st[0] *= m_texture_width;
    st[1] *= m_texture_height;

    Float distance = this->tblerp(st[0], st[1]);
    Vec<Float,2> st_x, st_y;
    texture_derivs(ray, hit, tbox, st, st_x, st_y);

    Float d_x1 = tblerp(st_x[0],st_x[1]);
    Float d_y1 = tblerp(st_y[0], st_y[1]);

    // forward difference of the distance value
    Float dfx = d_x1 - distance;
    Float dfy = d_y1 - distance;
    Float width = 0.7f * sqrt(dfx*dfx + dfy*dfy);
    Float res = smoothstep(0.5f-width,0.5f+width,distance);

    // super sample
    constexpr Float dscale = 0.354f;
    Vec<Float,2> ss_dy = dscale * (st_y - st);
    Vec<Float,2> ss_dx = dscale * (st_x - st);

    Vec<Float,2> ss = st - ss_dy - ss_dx;
    Float ss_0 = tblerp(ss[0], ss[1]);
    ss = st - ss_dy + ss_dx;
    Float ss_1 = tblerp(ss[0], ss[1]);
    ss = st + ss_dy - ss_dx;
    Float ss_2 = tblerp(ss[0], ss[1]);
    ss = st + ss_dy + ss_dx;
    Float ss_3 = tblerp(ss[0], ss[1]);
    ...
    m_text_color({0.f, 0.f, 0.f})
{
  ...
  Vec<Float,3> *anchor_ptr = m_anchors.get_host_ptr();
  Vec<Float,2> *offset_ptr = m_offsets.get_host_ptr();
  Vec<Float,2> *dims_ptr = m_dims.get_host_ptr();
  Vec<Float,2> *tcoords_ptr = m_tcoords.get_host_ptr();
  ...
  for(int i = 0; i < size; ++i)
  {
    ...
    Vec<Float,3> anchor = anchors[i];
    Vec<Float,2> width_height;
    ...
    Vec<Float,2> rot_pt = {{width_height[0] * offset[0],
                              width_height[1] * offset[1]}};
    float distance = 0.f;
    const Vec<Float,2> corners[4] = {
                                        {{            0.f, 0.f}},
                                        {{            0.f, width_height[0]}},
                                        {{width_height[1], 0.f}},
...
void Billboard::save_texture(const std::string file_name)
{
  ...
  Float *image_ptr = timage.get_host_ptr();
  for(int32 i = 0; i < size; ++i)
  {
    const int32 offset = i * 4;
    const Float val = m_texture.get_value(i);
...
void Billboard::shade(const Array<Ray> &rays, const Array<RayHit> &hits, Framebuffer &fb)
{
  ...
  const Vec<Float,3> text_color = m_text_color;
  ...
  RAJA::forall<for_policy>(RAJA::RangeSegment(0, hits.size()), [=] DRAY_LAMBDA (int32 ii)
  {
    ...
    if (hit.m_hit_idx != -1)
    {
      ...
      Vec<Float,4> fcolor = {{ text_color[0],
                                 text_color[1],
                                 text_color[2],
                                 hit.m_ref_pt[2]}};

      Vec<Float,4> color = d_framebuffer.m_colors[pixel_id];
...
Array<RayHit> Billboard::intersect (const Array<Ray> &rays)
{
  ...
  const Vec<Float, 4> *inner_ptr = m_bvh.m_inner_nodes.get_device_ptr_const ();
  ...
  RAJA::forall<for_policy> (RAJA::RangeSegment (0, size), [=] DRAY_LAMBDA (int32 i)
  {
    ...
    while (current_node != barrier)
    {
      if (current_node > -1)
      {
        ...
        {
          Vec<Float, 4> children = const_get_vec4f (&inner_ptr[current_node + 3]);
          int32 l_child;
      ...
      if (current_node < 0 && current_node != barrier) // check register usage
      {
        ...
        if(bill_hit.m_dist < closest_dist && bill_hit.m_dist > min_dist)
        {
          ...
          Float alpha = dbillboard.alpha(ray, bill_hit);
...
void Billboard::text_color(const Vec<Float,3> &color)
{
  m_text_color = color;
}
