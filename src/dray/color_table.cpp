                        ...
                        Array<Vec<float32, 4>> &colors) const
{
  ...
  {
    Vec<float32,4>  c = map_rgb(static_cast<float32>(i) / static_cast<float32>(num_samples - 1));
...
Vec<float32,4> ColorTable::map_rgb(float32 scalar) const
{
  ...
  const Vec<float32,4>& first_color = this->m_internals->m_rgb_points[first_color_index].m_rgba;
  const Vec<float32,4>& second_color = this->m_internals->m_rgb_points[second_color_index].m_rgba;
