...
void Kripke::Kernel::LPlusTimes(Kripke::Core::DataStore &data_store, const std::string &dest, const std::string &src)
{
  KRIPKE_TIMER(data_store, LPlusTimes);
  ...
  auto &field_phi_out =   data_store.getVariable<Field_Moments>(src);
  auto &field_rhs =       data_store.getVariable<Field_Flux>(dest);
  auto &field_ell_plus =  data_store.getVariable<Field_EllPlus>("ell_plus");
